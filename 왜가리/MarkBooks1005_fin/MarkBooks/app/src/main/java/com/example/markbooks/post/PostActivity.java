...
public class PostActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onClick(View view) {
        ...
        if(mAuth.getCurrentUser() != null){
            ...
            data.put("time", DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss").format(dateTime).toString());
            data.put(FirebaseID.timestamp, FieldValue.serverTimestamp()); //  시간별로 정렬
            mStore.collection(FirebaseID.post).document(postId).set(data , SetOptions.merge());
            //mStore.collection("user").document(id1).collection("post").document(postId).set(data);
            finish();
